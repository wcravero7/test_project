# NEWEST!!! USE THIS ONE. BUT MAKE SURE TO CHANGE THE CODE TO USE SKLEARN API
import dash
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        ], style={"border":"0px solid","width":"auto","height":"auto","marginLeft":"3vw","fontSize":"2vh"}),



        html.Button("Optimization", id="show-optimization-modal-btn",
              ...
              "clipPath":"inset(0px -5px 0px -5px)","display":"flex"}),
    ...
    dbc.Modal([
        ...
        dbc.ModalBody([
            ...
            dcc.RadioItems(
                options=['Interpolated', 'Historical'],
                value='Interpolated', inline=True, inputStyle={"marginRight":"1px"}, labelStyle={"marginRight":"20px"},
            ...
            ], id="interpolated-options-div", style={"display":"block"}),

            html.Div([
                html.Div("Top Percent of Samples to Use", style={"marginTop":"2vh", "marginBottom":"10px"}),
                dcc.Input(id="top-n", type="text", value="5", style={"width":"50px","marginLeft":"15px"}),
                html.Span("%"),
            ], id="historical-options-div", style={"display":"none"}),
    ...
    dbc.Modal([
        dbc.ModalHeader(dbc.ModalTitle("Optimization Comparison", id="optimization-comparison-modal-title")),
        dbc.ModalBody([
            dag.AgGrid(
                ...
                columnDefs=[
                    ...
                    {
                        "headerName":"Current Node",                        
                        "headerClass": "center-header",# "headerClass": "ag-header-cell-label-center",
                        "suppressStickyLabel": True,
                        "children":[
                            {"field":"Best", "headerName":"Best", "cellStyle":{"backgroundColor": "#6ff95a30"}},#"#f9a75a59" ##6ff95a57
                            {"field":"Best_Contribs", "headerName":"Contribs","cellStyle":{"backgroundColor": "#6ff95a30"}, "valueFormatter":{"function": "d3.format(',.2f')(params.value)"}}
                        ],
                    },
                    {
                        "headerName":"Other Node",
                        "headerClass": "center-header",# "headerClass": "ag-header-cell-label-center",
                        "suppressStickyLabel": True,
                        "children":[
                            {"field":"Worst", "headerName":"Worst", "cellStyle":{"backgroundColor": "#d359593d"}},#"#599ad358"
                            {"field":"Worst_Contribs", "headerName":"Contribs", "cellStyle":{"backgroundColor": "#d359593d"}, "valueFormatter":{"function": "d3.format(',.2f')(params.value)"}}
                        ]
                    },
                    # {
                    #     "headerName":"",
                ...
                columnSize="autoSize",#"sizeToFit" #autoSize
                style={"height":"100%", "width":"40%", "display":"inline-block", "verticalAlign":"top"}
            ),
            ...
            html.Div([
                ...
                html.Div(
                    dcc.Dropdown(
                        id="optimization-groupby-feature-dropdown",
                        multi=False,
                        clearable=True,
                ...
                , style={"width":"20vw","marginTop":"0px"}),

                dcc.Graph(
                    id="optimization-line-chart",
                    style={"position":"relative","height":"50vh","left":"0%","width":"100%","top":"0vh","border":"1px solid","padding":"5px", 
                        "boxSizing":"border-box"},
                    config={'displayModeBar':False}
                ),
            ], style={"marginLeft":"1%","width":"58%","display":"inline-block"})
    ...
    dcc.Store(id="optimized-values-store", data=[]),



    ###################### Data Preparation #####################
    html.Button("Data Preparation", id="data-preparation-btn",
                style={"position":"absolute","border":"none","right":"10vw","top":"0vh"}),
...
def get_graph(x, y, x_title, y_title, title, curr_x=None, curr_y=None):
    fig = go.Figure()
...
def update_optimization_line_chart(cell_data, categorical_feature, is_open):
    ...
    if feature in df.columns:
        if categorical_feature != None:
            if df[feature].dtype == object:
                contribs_chart = get_univariate_graph_of_categorical_feature_by_category(df, feature, categorical_feature, target, min_samples=0, max_categories=6)
            else:
                contribs_chart = get_univariate_graph_of_numerical_feature_by_category(df, feature, categorical_feature, target, frac=0.1, min_samples=0, max_points=5000, max_categories=6)
        else:
            if df[feature].dtype == object:
                contribs_chart = get_univariate_graph_of_categorical_feature(df, feature, target, min_samples=1000)
            else:
                contribs_chart = get_univariate_graph_of_numerical_feature(df, feature, target, frac=0.1, min_samples=0, max_points=5000)
        return contribs_chart
...
#     dash.dependencies.Input(component_id="optimized-values-store", component_property='data'),
#     dash.dependencies.State(component_id="optimization-comparison-table", component_property='columnDefs'),
#     prevent_initial_call=True,
# )
# def show_optimization_comparison_modal(optimized_values, columnDefs):
#     print("in show_optimization_comparison_modal")
#     optimized_values_df = pd.DataFrame(optimized_values)
#     return True, optimized_values_df.to_dict("records")
...
#     dash.dependencies.Output(component_id="synced-movement-checkbox", component_property='value'),

#     dash.dependencies.Output(component_id="optimized-values-store", component_property='data'),
...
#     prevent_initial_call=True,
# )
# def find_optimal_values(n_clicks, ordered_values, static_features, optimization_direction, feature_sliders, optimization_method, top_n, percentiles):
...
#     # Get all the current slider values from all features that will not be optimized.
...
#     X = pd.concat([df], axis=1).loc[:, selected_features].drop(columns=target)# X = df.drop(columns=target)
    

#     # Compute medians and modes for numerical and categorical features
...
#     if len(obj_cols) > 0:
#         # X[obj_cols] = X[obj_cols].fillna(X[obj_cols].mode().iloc[0])
#         mode_vals = X[obj_cols].mode().iloc[0]
...
#             if static_features[j] in obj_cols:
#                 filter_mask = filter_mask & (X[static_features[j]] == static_feature_values[j])
#             else:
#                 tol = X[static_features[j]].std(ddof=0) * 0.15
#                 filter_mask = filter_mask & (X[static_features[j]].between(static_feature_values[j] - tol, static_feature_values[j] + tol))
#         X = X.loc[filter_mask, :].reset_index(drop=True)
#         print(f"After filtering shape : {X.shape}")


#     if optimization_method == "Interpolated":
#         # Impute medians and modes into filtered dataframe
#         if len(num_cols) > 0:
#             X[num_cols] = X[num_cols].fillna(median_vals)
#         if len(obj_cols) > 0:
#             X[obj_cols] = X[obj_cols].fillna(mode_vals)


#         # Set Static feature values into dataframe
#         X[static_features] = static_feature_values


#         # Get initial solution #################################
#         numeric_summary = X.median(numeric_only=True)
#         categorical_summary = X.select_dtypes(include=["object"]).mode()#include=["object", "category"]
#         if len(categorical_summary) > 0:
#             categorical_summary = categorical_summary.iloc[0]
#         else:
#             categorical_summary = None
#         best_X = pd.DataFrame([pd.concat([numeric_summary, categorical_summary])])
#         best_X_encoded = pd.get_dummies(best_X, prefix_sep="?")
#         best_X_encoded = best_X_encoded.reindex(columns=model_features, fill_value=0)
...
#                 feature_values = np.unique(X[feature].dropna())
#                 cat_feature_cols = feature + "?" + feature_values
#                 model_ready_X = best_X_encoded.loc[best_X_encoded.index.repeat(len(feature_values))].reset_index(drop=True)
#                 model_ready_X[cat_feature_cols] = np.identity(len(cat_feature_cols))
#             else:
#                 feature_min, feature_max = np.nanpercentile(X[feature], q=percentiles)
#                 feature_values = np.unique(X.loc[X[feature].between(feature_min, feature_max), feature].dropna())
#                 model_ready_X = best_X_encoded.loc[best_X_encoded.index.repeat(len(feature_values))].reset_index(drop=True)
#                 model_ready_X[feature] = feature_values

#             if is_classification:
#                 predicted_vals = model.predict_proba(model_ready_X)[:,1]
#             else:
#                 predicted_vals = model.predict(model_ready_X)
#             # predicted_vals = model.inplace_predict(model_ready_X)
#             if optimization_direction == "Yes":
#                 optimal_prediction = np.max(predicted_vals)
#             else:
#                 optimal_prediction = np.min(predicted_vals)

#             optimal_indices = np.where(predicted_vals == optimal_prediction)[0]
#             optimal_index = optimal_indices[int(len(optimal_indices) / 2)]# the middle index is the optimal_index

#             best_X_encoded = model_ready_X.iloc[[optimal_index], :]
#         best_X_encoded.index = [0]


#         # Get optimal_X, which will be a decoded series with NO dummies
#         dummy_cols = best_X_encoded.filter(like="?")
#         decoded_categories = pd.from_dummies(dummy_cols, sep="?")
#         optimal_X = pd.concat([best_X_encoded.drop(columns=dummy_cols.columns), decoded_categories], axis=1).iloc[0,:]
#         print(optimal_X)
...
#         if is_classification:
#             # predictions = model.predict(xgb.DMatrix(model_ready_X))
...
#         optimal_index = np.argmin(pairwise_distances(top_standarized_samples, metric="nan_euclidean").mean(axis=1))
...
#         ######## NEWWWWWWWWWWWWWW ############################################################################################
#         # get bottom_n samples with lowest predicted values and find the sample that is most similar to the others
#         bottom_indices = sorted_indices[-int((top_n/100)*len(predictions)):]
#         bottom_samples = model_ready_X.iloc[bottom_indices, :]
#         bottom_predictions = predictions[bottom_indices]
#         bottom_standarized_samples = StandardScaler().fit_transform(bottom_samples)
#         optimal_index = np.argmin(pairwise_distances(bottom_standarized_samples, metric="nan_euclidean").mean(axis=1))

#         optimal_encoded_X = bottom_samples.iloc[[optimal_index], :].reset_index(drop=True)
#         dummy_cols = optimal_encoded_X.filter(like="?")
#         decoded_categories = pd.from_dummies(dummy_cols, sep="?")
#         optimal_X2 = pd.concat([optimal_encoded_X.drop(columns=dummy_cols.columns), decoded_categories], axis=1).iloc[0,:]
#         best_worst_df = pd.DataFrame({"Best":optimal_X, "Worst":optimal_X2}).reindex(ordered_features)
#         ######################################################################################################################
...
#     return False, optimal_values, [], best_worst_df.reset_index().to_dict("records")
...
def find_optimal_values(n_clicks, ordered_values, static_features, optimization_direction, feature_sliders, optimization_method, top_n, percentiles, columnDefs):
    print("in find_optimal_values")
    # static_features = ["Converting Machine", "Laminating Machine"]
    # static_feature_values = ["JJ2", "LAM2"]
    if static_features is None:
    ...
    if optimization_method == "Interpolated":
        ...
        dummy_cols = optimal_encoded_X.filter(like="?")
        decoded_categories = pd.from_dummies(dummy_cols, sep="?")
        optimal_X = pd.concat([optimal_encoded_X.drop(columns=dummy_cols.columns), decoded_categories], axis=1).round(3).iloc[0,:]
        best_contribs = model.get_booster().predict(xgb.DMatrix(optimal_encoded_X), pred_contribs=True)
        best_bias = best_contribs[0, -1]
        best_contribs = best_contribs[:, :-1]
        contrib_df = pd.DataFrame(best_contribs, columns=optimal_encoded_X.columns)
        contrib_df = contrib_df.groupby(lambda x: x.split('?')[0], axis=1).sum()
        best_ordered_contribs = np.asarray(contrib_df[ordered_features])[0]
        best_df = pd.DataFrame({"Best":optimal_X}).reindex(ordered_features)
        best_df["Best_Contribs"] = best_ordered_contribs
        best_prediction = best_ordered_contribs.sum() + best_bias
        if is_classification:
            best_prediction = (1 / (1 + np.exp(-best_prediction)))
            best_prediction = calibrate_probability(best_prediction)*100
        ...
        worst_encoded_X = bottom_samples.iloc[[optimal_index], :].reset_index(drop=True)
        dummy_cols = worst_encoded_X.filter(like="?")
        decoded_categories = pd.from_dummies(dummy_cols, sep="?")
        worst_X = pd.concat([worst_encoded_X.drop(columns=dummy_cols.columns), decoded_categories], axis=1).round(3).iloc[0,:]
        worst_contribs = model.get_booster().predict(xgb.DMatrix(worst_encoded_X), pred_contribs=True)
        worst_bias = worst_contribs[0, -1]
        worst_contribs = worst_contribs[:, :-1]
        worst_contrib_df = pd.DataFrame(worst_contribs, columns=worst_encoded_X.columns)
        worst_contrib_df = worst_contrib_df.groupby(lambda x: x.split('?')[0], axis=1).sum()
        worst_ordered_contribs = np.asarray(worst_contrib_df[ordered_features])[0]
        worst_df = pd.DataFrame({"Worst":worst_X}).reindex(ordered_features)
        worst_df["Worst_Contribs"] = worst_ordered_contribs
        worst_prediction = worst_ordered_contribs.sum() + worst_bias
        if is_classification:
            worst_prediction = (1 / (1 + np.exp(-worst_prediction)))
            worst_prediction = calibrate_probability(worst_prediction)*100


        best_worst_df = pd.concat([best_df, worst_df], axis=1).reset_index(names="Variable") #best_worst_df = pd.DataFrame({"Best":optimal_X, "Worst":worst_X}).reindex(ordered_features)
        ######################################################################################################################
    ...
    # columnDefs[1]["headerName"] = f"{target}: {filtered_df1[target].mean().round(3)} <br>Samples: {filtered_df1.shape[0]}"
    columnDefs[1]["headerName"] = best_prediction.round(1)
    columnDefs[2]["headerName"] = worst_prediction.round(1)
    return False, optimal_values, [], True, best_worst_df.to_dict("records"), columnDefs#False, optimal_values, [], best_worst_df.to_dict("records")
...
def clicked_button(n_clicks):
    ...
    # print("features from ordered_features")
    # print(ordered_features[0:15])
    # print()
...
def update_prediction(ordered_values, n_clicks, positive_contributions_div, negative_contributions_div, synced_movement):
    ...
    if (ctx.triggered_id["type"] == "click-features") and (np.any(n_clicks)):
        ...
        if selected_feature in categorical_features:
            ...
            model_ready_X[cat_feature_cols] = np.identity(len(cat_feature_cols))
            # temp_df = None################ new
            temp_df = model_ready_X.loc[model_ready_X[selected_feature + "?" + selected_feature_value] == 1, :].copy()################ new
        else:
            processed_feature_series = pd.concat([df], axis=1).loc[:, selected_feature].dropna()# Instead of concatenating, check in which df feature exists
            # feature_values = np.unique(df[selected_feature].dropna())
            feature_values = np.unique(processed_feature_series)
            if len(feature_values) > 5000:
                step_size = int(np.ceil(len(feature_values) / 5000))
                feature_values = feature_values[0::step_size]
            model_ready_X = model_ready_X.loc[model_ready_X.index.repeat(len(feature_values))].reset_index(drop=True)
            model_ready_X[selected_feature] = feature_values
            temp_df = model_ready_X.iloc[[0], :].copy()################ new
            # temp_df[selected_feature].iloc[0] = selected_feature_value############### new
            temp_df.at[temp_df.index[0], selected_feature] = selected_feature_value
        ...
        if is_classification:
            ...
            if target_map[1] == 1:
                ...
                y_display = f"{target_map[1]} (prob)"
            # contrib_fig = get_graph(x=feature_values, y=predicted_vals, x_title=selected_feature, y_title=y_display, title="")#title=f"{selected_feature} vs {y_display}"
            # if temp_df is not None:
            predicted_val_selected_feature = model.predict_proba(temp_df)[0,1]################ new
            # predicted_val_selected_feature = model.predict(xgb.DMatrix(temp_df))[0]################ new
            predicted_val_selected_feature = calibrate_probability(predicted_val_selected_feature)
            
            # contrib_fig = get_graph(x=feature_values, y=predicted_vals, x_title=selected_feature, y_title=y_display, title="", curr_x=selected_feature_value, curr_y=predicted_val_selected_feature)#title=f"{selected_feature} vs {y_display}"
            contrib_fig = get_both_graphs(x=feature_values, y=predicted_vals, feature=selected_feature, x_title=selected_feature, y_title=y_display, title="", curr_x=selected_feature_value, curr_y=predicted_val_selected_feature)
        else:
            predicted_vals = model.predict(model_ready_X)
            # predicted_vals = model.predict(xgb.DMatrix(model_ready_X))
            y_display = f"{target}*"
            # contrib_fig = get_graph(x=feature_values, y=predicted_vals, x_title=selected_feature, y_title=y_display, title="")#title=f"{selected_feature} vs {y_display}"
            # if temp_df is not None:
            predicted_val_selected_feature = model.predict(temp_df)[0]################ new
            # predicted_val_selected_feature = model.predict(xgb.DMatrix(temp_df))[0]################ new
            # contrib_fig = get_graph(x=feature_values, y=predicted_vals, x_title=selected_feature, y_title=y_display, title="", curr_x=selected_feature_value, curr_y=predicted_val_selected_feature)#title=f"{selected_feature} vs {y_display}"
            contrib_fig = get_both_graphs(x=feature_values, y=predicted_vals, feature=selected_feature, x_title=selected_feature, y_title=y_display, title="", curr_x=selected_feature_value, curr_y=predicted_val_selected_feature)
        ...
        return [dash.no_update]*len(positive_contributions_div), [dash.no_update]*len(negative_contributions_div), dash.no_update, True, contrib_fig, selected_feature, None, [dash.no_update]*len(ordered_values)
    ...
    # print("does the size match?")
    # print(len(local_ordered_features))
    # print(ordered_contribs.shape)
    # print("features from ctx.inputs_list")
    # print(local_ordered_features[0:15])
    # print(ordered_contribs[0:15])
    # print()
    ...
    if len(synced_values) > 0:
        return positive_contributions_div, negative_contributions_div, prediction_div, False, dash.no_update, dash.no_update, dash.no_update, synced_values
    else:
        return positive_contributions_div, negative_contributions_div, prediction_div, False, dash.no_update, dash.no_update, dash.no_update, [dash.no_update]*len(ordered_values)
