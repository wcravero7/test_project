...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div("PCA", id="pca-button", style={"display":"inline-block","marginLeft":"20px","border":"0px solid","color":"#599ad3","cursor":"pointer"}),
        ...
        dbc.ModalBody([
            ...
            html.Div([
                ...
                dcc.Graph(
                    ...
                    style={"position":"relative","height":"70vh","left":"0%","width":"100%","top":"0vh","border":"1px solid","padding":"5px", 
...
def get_contrib_graph_of_numerical_feature_by_category(df, feature, categorical_feature, contrib_df, max_points=5000, max_categories=5, ytitle=""):
    # combined_df = pd.DataFrame({"feature":df[feature], "categorical_feature":df[categorical_feature], "contrib":contrib_df[feature]}).dropna().sort_values("feature").reset_index(drop=True)
...
def click_next_sample(left_arrow_clicks, right_arrow_clicks, columnDefs):
    ...
    # curr_index = (right_arrow_clicks - left_arrow_clicks) % df.shape[0]
    # sample_df = df.iloc[[curr_index], :][selected_features].drop(columns=target)
    # numeric_cols =  sample_df.dtypes[sample_df.dtypes != "object"].index

    # curr_index = (right_arrow_clicks - left_arrow_clicks) % df.shape[0]
    # sample_df = df.drop(columns=target).iloc[[curr_index], :].copy()
    ...
    # result_df["feature"] = result_df["feature"] + "\n" + result_df["values"].map("{:.2f}".format)
    # result_df["contrib"] = result_df["contrib"] * 100
...
def update_predictions(slider_value, categorical_value, rowData, columnDefs, selected_data):
    ...
    for i in range(0, len(features_list)):
        ...
        if feature == selected_feature:
            ordered_values[i] = updated_value#slider_value# sets the selected feature value to the value in the slider
    ...
    # Cast model_ready_X to the correct data types
    ...
    # set the selected feature value to the selected slider value
...
def find_optimal_values(n_clicks, ordered_values, static_features, optimization_direction, feature_sliders, optimization_method, top_n, percentiles, columnDefs):
    ...
    print(best_prediction.round(1))
    print(worst_prediction.round(1))
